...
export class NivelDetalleCursoComponent implements OnInit {
  ...
  cursos: Curso[] = [];
  showForm: boolean = true; // Controls the toggle between form and list
  ...
  fetchNivelDetalleCursos(): void {
    this.nivelDetalleCursoService.getAll().subscribe({
      ...
      error: (err) => console.error('Error fetching nivel detalle cursos:', err)
    });
  }
  ...
  fetchNivelesDetalles(): void {
    this.nivelDetalleService.getAll().subscribe({
      ...
      error: (err) => console.error('Error fetching niveles detalles:', err)
    });
  }
  ...
  createOrUpdate(): void {
    ...
    request.subscribe({
      next: () => {
        this.fetchNivelDetalleCursos(); // Refresh list
        this.resetForm();
        this.showForm = false; // Switch to list view after create/update
      },
      error: (err) => console.error('Error during create/update:', err)
    });
  }
  ...
  deleteCurso(id: number): void {
    if (confirm('¿Quiere eliminarlo?')) {
      this.nivelDetalleCursoService.delete(id).subscribe({
        next: () => this.fetchNivelDetalleCursos(), // Refresh list
