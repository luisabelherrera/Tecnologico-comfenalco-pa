...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Future<void> _loadAllData() async {
    ...
    try {
      final results = await Future.wait([
        ...
        _apiService.getAllNivelesDetalles(),
        _apiService.getAllPeriodos(),
        _apiService.getAllNiveles(),
      ]);

      _inscripciones = results[0];
      _calificaciones = results[1];
      _nivelesDetalles = results[2];
      _periodos = results[3];
      _niveles = results[4];
  ...
  void _calculateSummaryStats() {
    final uniqueEstudiantes = _inscripciones.map((ins) => ins['estudiante']?['idEstudiante']).toSet();
    ...
    if (_calificaciones.isNotEmpty) {
      final sum = _calificaciones.fold<double>(0, (prev, element) => prev + (element['nota'] ?? 0.0));
  ...
  void _processTrendStats() {
    ...
    for (var cal in _calificaciones) {
      ...
      final periodoId = cal['curricular']?['docenteNivelDetalleCurso']?['nivelDetalleCurso']?['nivelDetalle']?['nivel']?['idPeriodo'] ?? 1;
    ...
    for (var key in sortedKeys) {
      final avg = periodCounts[key]!.reduce((a, b) => a + b) / periodCounts[key]!.length;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Reportes y Estadísticas', style: TextStyle(fontWeight: FontWeight.bold)),
                            ...
                            sideTitles: SideTitles(
                              ...
                              getTitlesWidget: (val, meta) {
                                if (val.toInt() < _trendLabels.length) {
                                  return Text(_trendLabels[val.toInt()], style: const TextStyle(fontSize: 10));
                                }
                                return const Text('');
                              },
                            ),
        ...
        children: [
          Row(
            children: [
              ...
              const Text('Análisis IA (Gemini)', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              const Spacer(),
              if (_aiLoading) const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)),
            ],
          ),
          const SizedBox(height: 8),
          Text(_aiInsight, style: const TextStyle(color: Colors.white70, fontSize: 13, height: 1.4)),
        ],
      ),
    );
  }
  ...
  Widget _buildSummaryCards() {
    return GridView.count(
      ...
      crossAxisSpacing: 12,
      children: [
        _buildStatCard('Estudiantes', _totalEstudiantes.toString(), Icons.people, Colors.blue),
        _buildStatCard('Promedio', _promedioGeneral.toStringAsFixed(1), Icons.star, Colors.orange),
        _buildStatCard('Matrículas', _totalInscripciones.toString(), Icons.assignment, Colors.green),
        _buildStatCard('Niveles', _totalNiveles.toString(), Icons.layers, Colors.purple),
      ],
    );
  }
  ...
  Widget _buildChartCard({required String title, required String subtitle, required double height, required Widget chart}) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
